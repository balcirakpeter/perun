	...
	protected void initialize() throws PerunException {
		...
		try {
			...
			try {
				...
				rights.add(new AttributeRights(attrDef.getId(), Role.SELF, Arrays.asList(new ActionType(ActionType.READ, "User"))));
		...
		try {
			...
			try {
				...
				// set attribute rights
				List<AttributeRights> rights = new ArrayList<AttributeRights>();
				rights.add(new AttributeRights(attrDef2.getId(), Role.SELF, Arrays.asList(new ActionType(ActionType.READ, "User"))));
				perun.getAttributesManager().setAttributeRights(cabinetSession, rights);
			} catch (PerunException pe) {
