	...
	public Attribute getAttributeValue(PerunSessionImpl sess, Member member, Resource resource, AttributeDefinition attributeDefinition) throws InternalErrorException {
		...
		if(resourceQuotas == null || resourceQuotas.getValue() == null) resourceTransferedQuotas = new HashMap<>();
			...
			try {
				...
				resourceTransferedQuotas = sess.getPerunBl().getModulesUtilsBl().transferQuotas(resourceQuotas, resource, null, false);
		...
		if(memberQuotas == null || memberQuotas.getValue() == null) memberTransferedQuotas = new HashMap<>();
			...
			try {
				...
				memberTransferedQuotas = sess.getPerunBl().getModulesUtilsBl().transferQuotas(memberQuotas, resource, member, false);
			} catch (WrongAttributeValueException ex) {
		...
		if(memberQuotasOverride == null || memberQuotasOverride.getValue() == null) memberTransferedQuotasOverride = new HashMap<>();
			...
			try {
				...
				memberTransferedQuotasOverride = sess.getPerunBl().getModulesUtilsBl().transferQuotas(memberQuotasOverride, resource, member, false);
			} catch (WrongAttributeValueException ex) {
